  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      if(address==='' || phoneNumber===''){
        // Show alert if no delivery info entered
        Alert.alert(
            "Note",
        ...
        if (userSnapshot.exists()) {
            ...
            if (userData && userData.myCart) {
                ...
                // Deduct ordered quantity from Products collection
                selectedItems.forEach(async (item) => {
                    const productRef = doc(firestore, "Products", item.id);
                    const productSnapshot = await getDoc(productRef);
                    if (productSnapshot.exists()) {
                        const productData = productSnapshot.data();
                        const updatedQty = productData.qty - item.qtyOrdered;
                        batch.push(updateDoc(productRef, { qty: updatedQty }));
                    }
                });
  ...
  return (
    <ScrollView>
      ...
      <View className="mx-2">
        ...
        <Text className="font-bold text-xl">Delivery information <Text style={{color: "red", fontSize:12}}>(required)</Text></Text>
        {hasDeliveryInfo ? (
          <View>
